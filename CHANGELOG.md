...
Read `docs/upgrade-contract.md` first. The short version:

1. **Stop the old plugin from writing.** Migration from a live database is refused.
2. **Back up the old store yourself.** The tool keeps its own copy, but take one anyway. It is the only irreplaceable thing here.
3. **Install 3.1.0 fresh** and let the installer create an empty instance. Do not point the new plugin at the old directory.
4. **Prepare the job:**
   ...
   If the old store has more than one scope, add `--scope-map` to map each one to an audience the new instance is bound to. Migration will not guess, will not merge two scopes into one audience, and will not widen a permission it cannot read.
...
6. **Queue the index** once you are happy with what came across:
...
- **`blocked` is an answer, not something to retry around.** Your old database is intact and the new one is explicitly incomplete. Do not start using it until the block is resolved.
- **Migration never calls a model.** Nothing leaves your machine during the job. Only the index step afterwards spends anything.
- **Old vectors are reused only if they can be proven equivalent** — same model, dimensions, encoding, segmentation and source mapping. Otherwise the index is built fresh. Matching row counts are not evidence that memories match.
- **Anything that cannot be converted losslessly goes to an archive**, and the report tells you. We would rather park a memory in cold storage than quietly change its meaning.
