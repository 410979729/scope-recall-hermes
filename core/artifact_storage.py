...
class Artifacts:
    ...
    def get(self, ref, revision=None):
        ...
        row = conn.execute(
            f"""SELECT a.*,v.* FROM artifacts a JOIN artifact_versions v ON v.artifact_id=a.artifact_id
        ...
        return Artifact(
            ...
            row["retention_state"],
            blob,
            tuple(json.loads(row["description_json"])),
            refs,
            bool(row["suppressed"]),
        )
    ...
    def register(
        ...
        *,
        ...
        if source is None or (source.scope_id, source.project_id, source.branch_id) != (
            scope_id,
            ctx.project_id,
            ctx.branch_id,
        ):
        ...
        if existing:
            if (
                existing.sha256 != (grant.sha256 if grant else None)
        ...
        if head is None:
            conn.execute(
                "INSERT INTO artifacts(artifact_id,scope_id,project_id,branch_id,current_revision,suppressed) VALUES (?,?,?,?,?,?)",
                (
                    ref,
                    scope_id,
                    ctx.project_id,
                    ctx.branch_id,
                    revision,
                    int(source.suppressed),
                ),
            )
        else:
            conn.execute(
                "UPDATE artifacts SET current_revision=?,suppressed=max(suppressed,?) WHERE artifact_id=?",
        ...
        conn.execute(
            """INSERT INTO artifact_versions(artifact_id,revision,label,media_type,sha256,size_bytes,retention_state,relative_path,blob_json,description_json,recorded_at)
            VALUES (?,?,?,?,?,?,?,?,?,?,?)""",
            (
                ref,
                revision,
    ...
    def open(self, ref, revision):
        ...
        if artifact.blob is None:
            raise ContractError("SOURCE_MISSING", "artifact_not_retained")
