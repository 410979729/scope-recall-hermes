# Scope Recall 1.10.2 Release Readiness

Date: 2026-08-21

Owner: maintainers.

This public maintainer note records the source-candidate requirements for the `1.10.2` public tree since the last tagged and packaged public release, `1.9.2`. It incorporates and supersedes the untagged `1.10.0` public source candidate and the `1.10.1` public source candidate. This document excludes environment-specific health counters, filesystem locations, credentials, and private validation context. No commit, tag, GitHub Release, PyPI publication, deployment, or live migration is implied by this document.

## Candidate scope

- `1.10.2` carries forward the complete `1.10.1` feature and compatibility scope documented in [`release-readiness.1.10.1.md`](release-readiness.1.10.1.md).
- The verified online-backup cleanup fixture writes a simulated external staging replacement through an independent stdlib SQLite connection so it does not enter this process's truth-connection hardening cache. This is a POSIX CI fixture correction. It does not weaken descriptor hardening or change production runtime behavior.
- Windows recovery-command test diagnostics decode CP936/GBK before permissive OEM fallback. This is a Windows CI fixture correction in test helpers only. Production recovery command generation and runtime behavior are unchanged.
...
- Package/plugin version: `1.10.2`.
- Public release baseline: `1.9.2`.
- Expected annotated release tag: `v1.10.2` (not created by this source-candidate task).
- Published `1.9.2` source, tag, and package artifacts remain immutable.
- The `1.10.0` and `1.10.1` public source candidates were pushed without a tag, GitHub Release, or PyPI artifact; `1.10.2` supersedes those source bytes.
...
- Ruff, Pyright, the focused journal-health, truth-connection/writer-lease permission, descriptor-hardening, verified online-backup cleanup, and version-identity suites, plus `git diff --check`, must pass on one frozen candidate.
