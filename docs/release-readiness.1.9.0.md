...
- Build, activation, and rollback acquire the durable activation lease, then hold one `BEGIN IMMEDIATE` writer fence across source binding, the online backup (copied through a separate reader connection), the post-backup binding compare, and guard-trigger installation; the backup never contains temporary guard triggers, and permanent race tests inject raw writers at all four boundaries. Activation rechecks the reviewed source binding in addition to the explicit current-generation compare-and-swap. Rollback changes only the pointer and retains both legacy and shadow index storage.
- Shadow FTS rows and bigram postings share the truth-row integer docid, so trigger and backfill identity maintenance is indexed. Two-character Chinese concepts use the indexed postings channel with a covering-index document-frequency prefilter that drops corpus-wide terms; a single generic bigram is insufficient final relevance evidence.
- The release gate runs a fixed 50,000-row release-contract benchmark and enforces CJK 3/3 discovery, zero English candidate removal, requested-limit compliance, `p95 <= 100 ms` shadow latency, and bounded SQLite page growth. A held-out Chinese golden set covers synonym rewrites, typos, homophone and near-shape confusions, high-frequency interference, negation, lifecycle-hidden rows, scope isolation, and forbidden IDs with MRR, nDCG, Precision@k, and false-positive reporting plus explicit legacy-versus-shadow channel attribution in vector-off and vector-on configurations.
