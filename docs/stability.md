...
- writable POSIX truth connections raw-open and fchmod-harden each live database identity at most once per process; a later same-process open must not close a raw descriptor on that inode. Identity replacement or permission drift after that cached hardening event fails closed instead of raw-opening while this process may hold SQLite locks. An incompatible or foreign process-wide hardening marker fails closed and requires a process restart; it is not repaired into trusted cache evidence. Windows keeps inherited ACL behavior and does not use descriptor chmod
