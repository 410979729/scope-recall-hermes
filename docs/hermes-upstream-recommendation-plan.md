...
Status after the `v1.1.1` installer/distribution and journal-quality patch work. Completed items must still be re-verified immediately before tag/release/upstream PR, but this is the target evidence shape for proposed standalone-provider visibility.

### Repository hygiene

- [ ] Current follow-up branch working tree is clean before merge/release.
- [ ] Latest git tag agrees with `pyproject.toml`, `plugin.yaml`, README version text, and `CHANGELOG.md` after the `v1.1.1` release tag is created.
