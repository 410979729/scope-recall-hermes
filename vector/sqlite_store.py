...
class SQLiteBruteForceVectorStore(VectorStore):
    ...
    def fenced_upsert_records(
        ...
        """Upsert a whole group only while the caller's lease still holds.

        The worker's embed path always publishes through this fenced form, so a
        companion without it cannot be published to at all: every embed item
        fails and the index stays empty.  The LanceDB companion fences the write
        inside its native lock; here the store's own lock is that boundary, so
        the guard is evaluated with the write already serialized and a refused
