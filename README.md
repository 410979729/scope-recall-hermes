...
This checkout is release candidate `3.1.0rc38`. A candidate keeps its own name
when an evaluation writes it differently, and an attempt no process finished is
offered once more instead of waiting forever for evidence it already has. rc37
made a recalled question reach the reply its turn was given, and one name written
two ways -- DLSS5 and DLSS 5 -- one name. rc36 kept a consolidation's claims when
a summary beside them did not qualify and let a worker pass find the truth writer
...
The current release candidate is `3.1.0rc38`; these commands are local
