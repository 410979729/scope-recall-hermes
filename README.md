...
Version `1.1.1` continues the stable V1 release line for the documented interfaces, packaged as a public release candidate for broader field testing. It keeps the V1 compatibility contract in [`docs/stability.md`](docs/stability.md), preserves the supported standalone install shape added in v1.1.0, and tightens journal quality gates so assistant-only acknowledgements cannot become durable memories. It also keeps native-safe LanceDB probing and automatic SQLite vector fallback for non-AVX hosts.
