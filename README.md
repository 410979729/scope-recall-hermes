...
Version `1.10.0` is the public source candidate since `1.9.2`. It adds dry-run/epoch/backup/ledger journal source restore, bounded unresolved-journal retry/quarantine with fair budget deferral, a structured non-activatable inactive READY vector inventory, and one assembled production command port. It incorporates the unpublished `1.9.3` source interval (writer lease, read-only followers, digest transactions outside writes, and idle peer recovery) without changing stable provider/tool identities or SQLite authority. This tree is a source candidate on `main` only.
...
Release `1.10.0` is the public source candidate since `1.9.2`:
