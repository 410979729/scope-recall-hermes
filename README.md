...
Version `1.10.4` is the public patch candidate on the last packaged `1.10.3` line. It hardens issue #50 follow-up behavior: rollback restores exact recorded metadata and rejects malformed evidence; governance coverage requires a recognized latest receipt that still matches the current archive; Experience feedback can close an identified pending `run_id`; and the successful `memory_auto_adjudication` throttle survives provider recreation. SQLite authority, stable provider/tool identities, and the database schema are unchanged.
...
Release `1.10.4` is the follow-up patch candidate on the last packaged `1.10.3` line:
