...
Version `1.10.2` is the public source candidate since the last tagged and packaged public release `1.9.2`. It incorporates and supersedes the untagged `1.10.0` public source candidate and the `1.10.1` public source candidate. It corrects two CI fixtures and does not change production runtime behavior: a simulated external staging replacement does not enter this process's truth-connection hardening cache, without weakening descriptor hardening; and Windows recovery-command test diagnostics decode CP936/GBK before permissive OEM fallback. It retains POSIX truth-database descriptor caching so later writable connections cannot cancel same-process SQLite locks, and keeps journal deferred-metric doctor fixtures independent of the default backlog-age failure policy, without changing stable provider/tool identities or SQLite authority. This tree is a source candidate on `main` only.
...
Release `1.10.2` is the public source candidate since the last tagged and packaged public release `1.9.2`. It incorporates and supersedes the untagged `1.10.0` public source candidate and the `1.10.1` public source candidate:
