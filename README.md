...
Version `1.10.1` is the public source candidate since the last tagged and packaged public release `1.9.2`. It incorporates and supersedes the untagged `1.10.0` public source candidate. It hardens POSIX truth-database descriptor caching so later writable connections cannot cancel same-process SQLite locks, and keeps journal deferred-metric doctor fixtures independent of the default backlog-age failure policy, without changing stable provider/tool identities or SQLite authority. This tree is a source candidate on `main` only.
...
Release `1.10.1` is the public source candidate since the last tagged and packaged public release `1.9.2`. It incorporates and supersedes the untagged `1.10.0` public source candidate:
