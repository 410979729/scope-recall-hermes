...
- The single production recall path compiles one Recall Packet with current-truth selection, conflict exposure, evidence ordering, and bounded token budgeting.
- Purge is deny-first and two-phase; governed tool profiles keep the default core surface compact; optional extension boundaries remain disableable and non-authoritative.
- The developer-only Recall Inspector explains the exact production packet without direct private-table access or query-side writes.
- Release, migration, rollback, read-only, writer, and packaging rehearsals remain isolated from active Hermes instances.

Release `1.10.6` was the final 1.x stabilization candidate and superseded the unpublished `1.10.4` and `1.10.5` source checkpoints:
...
- PyPI provenance requires an exact release workflow run that completed successfully, without broadening source-executing job permissions.
- Merge/capture serialization, L4 retry retention, contradiction-chain selection, and distribution scanner boundaries preserve accepted work without hiding unsafe artifacts.
- The cumulative `1.10.4` rollback, governance, Experience feedback, and adjudication-throttle fixes remain included.

Release `1.10.4` was the preceding unpublished source checkpoint:

- Cleanup rollback replaces archived metadata with the validated before-snapshot and fails closed on malformed rollback evidence.
- Governance coverage trusts only explicit archive event/action pairs and only when the latest receipt still binds the current archived row.
- Experience preflight rows remain pending until feedback supplies their optional `run_id`; terminal playbooks may close an existing pending run without changing terminal counters.
- The successful `memory_auto_adjudication` throttle is persisted in the governance ledger, while failed runs remain retryable.
