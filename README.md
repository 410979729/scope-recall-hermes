...
Version `1.9.3` is the compatibility-preserving patch candidate since `1.9.2`. It enforces one cross-process writer per SQLite truth store, degrades additional providers to read-only followers, moves digest model calls outside write transactions, and recovers one idle same-process dirty peer without changing stable provider/tool identities or SQLite authority.
...
Release `1.9.3` is the compatibility-preserving patch candidate since `1.9.2`:
