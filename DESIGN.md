...
Default policy is conservative for gateways: global curated files are read in single-user/no-`user_id` contexts, but explicit `user_id` contexts must opt in via `curated_memory.mode: profile-global` or `curated_memory.mode: explicit-users` plus `allowed_user_ids`. Legacy `curated_memory.mode: shared` remains a deprecated compatibility alias for `profile-global`; new configuration UIs should expose only `single-user`, `explicit-users`, `profile-global`, and `disabled`.
