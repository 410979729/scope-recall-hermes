...
STATE = ROOT / ".execution/TEST-P07-failure-resume-v4"
...
def prepare() -> None:
    ...
    raw = [
        ...
        dict(protocol_version="1.1", source_event_key="TEST-P07-failure/2", source_revision=1, origin="tool_observation", role="tool", content=command['stdout']+'\n'+command['stderr']+f"\nexit_code: {command['exit_code']}", occurred_at=tool_at, recorded_at=tool_at, time_precision="instant", capture_state="complete", evidence_refs=[]),
...
def extract(model='mimo-v2.5') -> None:
    ...
    result = runtime.send(model=model, messages=messages, response_format={"type": "json_object"})
    record = dict(phase="extract",model=model,ledger_id=result.ledger_id, status=result.status, error_type=result.error_type, usage=result.usage, source_refs=prepared["source_refs"], source_inputs=source_inputs, accepted=False, rejected=False, rejection_reason=None, extracted_at=now())
...
if __name__ == "__main__":
    parser=argparse.ArgumentParser();parser.add_argument('phase',choices=('prepare','extract','reopen','answer'))
    parser.add_argument('--extractor',choices=('mimo-v2.5','deepseek-v4-flash'),default='mimo-v2.5');args=parser.parse_args()
    if args.phase=='extract':extract(args.extractor)
    else:{'prepare':prepare,'reopen':reopen,'answer':answer}[args.phase]()
