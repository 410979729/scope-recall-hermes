...
def hard_delete_memories(
    ...
    *,
    ...
    """Commit truth deletion and durable vector intent before external I/O.

    SQLite truth, governance audit, and one delete outbox event per memory are
    committed atomically. Only then may ``vector_delete`` touch the external
    companion. A crash before/during that callback therefore leaves deleted
    truth plus replayable delete intent.
    """

    clean_ids = list(dict.fromkeys(str(memory_id).strip() for memory_id in memory_ids if str(memory_id).strip()))
    if not clean_ids:
        return {"ok": True, "deleted": 0, "ids": [], "event_ids": [], "outbox_enqueued": 0}
    ...
    try:
        ...
        clean_scope_ids = list(dict.fromkeys(str(scope_id).strip() for scope_id in (scope_ids or []) if str(scope_id).strip()))
        ...
        if require_vector_delete:
            ...
            if vector_delete is not None:
                try:
                    vector_delete(scoped_ids)
                    # Keep the durable event pending until the ordinary outbox
                    # worker acknowledges this idempotent companion mutation.
                    vector_status = "applied_pending_ack"
        ...
        return {
            ...
            "vector_pending": vector_status in {"pending", "applied_pending_ack"},
...
def _snapshot(row: sqlite3.Row, *, metadata: Mapping[str, Any], relations: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "id": str(row["id"]),
        "scope_id": str(row["scope_id"] or ""),
...
def _enqueue_vector_transition(
    ...
    *,
    ...
    enqueue_vector_event(
        ...
        memory_id=memory_id,
        operation=operation,
        payload={"updated_at": updated_at, "reason": sanitize_report_text(reason)[:500]},
    )
    return True
...
def transition_memory_lifecycle(
    ...
    *,
    ...
    try:
        ...
        if expected_lifecycle and current_lifecycle != str(expected_lifecycle).strip().lower():
            raise LifecycleConflictError(
        ...
        after_metadata = safe_after_metadata if isinstance(safe_after_metadata, dict) else {"lifecycle": lifecycle}
        after_metadata["lifecycle"] = lifecycle
        cursor = conn.execute(
        ...
        if cursor.rowcount != 1:
            fresh = conn.execute("SELECT updated_at, metadata FROM memories WHERE id = ?", (memory_id,)).fetchone()
            fresh_updated_at = str(fresh["updated_at"] or "") if fresh is not None else ""
            fresh_lifecycle = str(load_metadata(fresh["metadata"] if fresh is not None else "{}").get("lifecycle") or "active")
            raise LifecycleConflictError(
        ...
        if hidden:
            ...
            upsert_memory_freshness(conn, memory_id=memory_id, metadata=after_metadata, commit=False)
        ...
        generation_id = _current_generation_id_read_only(conn)
        outbox_enqueued = _enqueue_vector_transition(
            conn,
            generation_id=generation_id,
            memory_id=memory_id,
