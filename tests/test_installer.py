...
def test_distribution_metadata_exposes_official_standalone_install_shape():
    ...
    assert pyproject["project"]["version"] == "1.10.2"
...
def test_confirmed_activation_compensation_refuses_post_snapshot_truth_drift(
    ...
    assert receipt["sqlite"]["drift_detected"] is True
    assert type(receipt["sqlite"]["drift_detected"]) is bool
    assert receipt["sqlite"]["manual_recovery_required"] is True
    assert type(receipt["sqlite"]["manual_recovery_required"]) is bool
...
def test_activation_guard_precedes_online_backup_and_backup_is_guard_free(
    ...
    assert receipt["sqlite"]["drift_detected"] is False
    assert type(receipt["sqlite"]["drift_detected"]) is bool
    assert receipt["sqlite"]["manual_recovery_required"] is False
    assert type(receipt["sqlite"]["manual_recovery_required"]) is bool
...
def test_installer_upgrade_backs_up_existing_plugin_and_reports_versions(tmp_path):
    ...
    assert result["manifest_version"] == "1.10.2"
    assert result["new_version"] == "1.10.2"
    ...
    assert "version: 1.10.2" in (plugin_dir / "plugin.yaml").read_text(encoding="utf-8")
...
def test_installer_rollback_restores_backup_and_backs_up_current_plugin(tmp_path):
    ...
    assert rollback["replaced_version"] == "1.10.2"
    current_backup = Path(rollback["current_backup_path"])
    assert current_backup.is_dir()
    assert "version: 1.10.2" in (current_backup / "plugin.yaml").read_text(encoding="utf-8")
