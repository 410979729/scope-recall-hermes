...
This is the production identity/policy source for canonical 2.0 profile
...
TOOL_SPECS: tuple[ToolSpec, ...] = (
    ToolSpec(
        "scope_recall_store",
        SCOPE_RECALL_STORE_SCHEMA,
        frozenset({"core", "compatibility", "maintenance", "developer", "extension"}),
        "write",
        "C03",
        aliases=("lancepro_store",),
    ),
    ToolSpec(
        "scope_recall_search",
        SCOPE_RECALL_SEARCH_SCHEMA,
    ...
    ToolSpec(
        "scope_recall_memory",
        SCOPE_RECALL_MEMORY_SCHEMA,
        frozenset({"core", "maintenance", "developer", "extension"}),
        "conditional",
        "C03",
    ),
    ToolSpec(
    ...
    ToolSpec(
        "scope_recall_probe",
        SCOPE_RECALL_PROBE_SCHEMA,
        frozenset({"compatibility", "developer"}),
        "read",
        "C04",
    ),
    ToolSpec(
    ...
    ToolSpec(
        ...
        "write",
        "C16",
        feature_gate="secret_index",
    ),
    ToolSpec(
        "scope_recall_playbook_search",
        SCOPE_RECALL_PLAYBOOK_SEARCH_SCHEMA,
        frozenset({"compatibility", "extension"}),
        "read",
        "C14",
        feature_gate="experience",
    ),
    ToolSpec(
    ...
    ToolSpec(
        ...
        "write",
        "C14",
        feature_gate="experience+maintenance",
    ),
    ToolSpec(
...
def visible_tool_specs(*, profile: str, flags: set[str]) -> list[ToolSpec]:
    """Apply profile selection and independent permission/capability gates."""
    ...
    for spec in TOOL_SPECS:
        ...
        if gate == "experience":
            if "experience" not in flags:
                continue
            if profile in spec.surfaces or spec.name in flags:
                visible.append(spec)
            continue
