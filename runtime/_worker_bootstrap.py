...
def main() -> int:
    if len(sys.argv) not in (2, 3) or sys.stdin.buffer.read(1) != b"\x01":
        return 125
    sys.stdin.close()
    from scope_recall.runtime.worker_entry import run_worker

    deadline_epoch = float(sys.argv[2]) if len(sys.argv) == 3 else None
    return run_worker(sys.argv[1], deadline_epoch=deadline_epoch)
