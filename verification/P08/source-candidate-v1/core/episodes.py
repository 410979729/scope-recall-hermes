...
UNSETTLED = re.compile(r'假设|假如|如果|也许|可能|引用|据说|他说|她说|suppose|hypothetical|perhaps|maybe|quoted',re.I)
...
def state_from_sources(sources, *, has_goal=False, previous='unknown') -> str:
    ...
    for source in ordered:
        if source.event['occurred_at'] is None and state!='unknown':continue
        ...
        if UNSETTLED.search(raw): continue
        if source_origin(source)=='host_generated':
            ...
            try: lifecycle=json.loads(raw)
            except (ValueError,TypeError): continue
            if type(lifecycle) is dict and lifecycle.get('lifecycle')=='interrupted': state='interrupted'
            continue
        if AUTHORITY_QUESTION.search(raw):
            continue
        cancelled=re.search(r'(?:取消|cancel(?:led)?)(?:整个)?(?:此|这个|该|the |this )?(?:任务|task)|任务(?:已)?取消',raw,re.I)
        failed=re.search(r'失败|exit.code["\s:：=]*[1-9]\d*|\bfailed\b',raw,re.I)
        completed=re.search(r'(?:整个任务|全部工作|任务)(?:已经|已|全部)?完成|(?:whole task|all work) (?:is |was )?completed',raw,re.I)
        if cancelled and preserves_qualifiers(raw,cancelled.group()):
            state='cancelled'
        elif failed and preserves_qualifiers(raw,failed.group()): state='failed'
        elif completed and preserves_qualifiers(raw,completed.group()) and not UNFINISHED.search(raw): state='completed'
        elif re.search(r'暂停|先停|继续|pause|resume|continue',raw,re.I): state='open'
        ...
        for item in values:
            ...
            if field in {'decisions','verified_progress'}:
                ...
                for source in candidates:
                    ...
                    if UNSETTLED.search(source.event['content']) or AUTHORITY_QUESTION.search(source.event['content']): continue
                    if field=='verified_progress' and any(UNFINISHED.search(c) or not FINISHED.search(c) for c in contexts): continue
