...
def ordinal(mention,count):
    ...
    if match:
        value=match[1]; index=int(value) if value.isdigit() else '一二三四五六七八九十'.index(value)+1
        return index-1 if 1<=index<=count else None
    if '中间' in mention and count%2==1: return count//2
    if '最后' in mention and count:return count-1
    return None
...
class References:
    ...
    def get(self,ref,revision=None):
        ...
        if not allowed(self.tx,'reference',ref):return None
        scopes=sorted(ctx.allowed_scope_ids)
        row=conn.execute(f'''SELECT r.*,v.* FROM reference_bindings r JOIN reference_versions v ON v.reference_id=r.reference_id
        ...
        if any(self.tx.source(*parse_source_ref(s)) is None for s in payload['evidence_refs']):return None
        if revision is None:
            for source in payload['evidence_refs']:
                try:self.tx.claims.require_live_source(*parse_source_ref(source))
                except ContractError:return None
        return Reference(ref,row['revision'],row['scope_id'],row['project_id'],row['branch_id'],payload,row['qualification_reason'],bool(row['suppressed']))
    ...
    def _stored_head(self, ref):
        ...
        """
        row = self.tx._check().execute('''SELECT r.current_revision,v.payload_json
            FROM reference_bindings r JOIN reference_versions v
            ON v.reference_id=r.reference_id AND v.revision=r.current_revision
            WHERE r.reference_id=? AND r.read_blocked=0''',(ref,)).fetchone()
    ...
    def _visible_artifact_identity_count(self, label, revision):
        ...
        ctx=self.tx.context
        scopes=sorted(ctx.allowed_scope_ids)
        return self.tx._check().execute(f'''SELECT count(DISTINCT a.artifact_id) FROM artifacts a JOIN artifact_versions v
            ...
            (label,revision,*scopes,ctx.project_id,ctx.branch_id)).fetchone()[0]
    ...
    def _explicit_candidate_mention(raw, label, revision, candidate, mention):
        ...
        if UNSETTLED.search(raw) or AUTHORITY_QUESTION.search(raw):return False
        pieces=[p for p in re.split(r'[，,;；。!?！？\n]|\.(?:\s|$)',raw) if p]
        version=rf'(?:v\s*{revision}(?![.\d])\b|第\s*{revision}\s*版)'
        marker=re.compile(re.escape(label)+rf'\s*(?:的|版本)?\s*{version}',re.I)
        if re.search(version+r'$',label,re.I):marker=re.compile(re.escape(label))
        def linked(piece,start,end):
            for occurrence in re.finditer(re.escape(mention),piece):
                if start<=occurrence.start()<end or occurrence.start()<=start<occurrence.end():return True
                if occurrence.end()<=start and re.fullmatch(r'[\s“”‘’\"\']*(?:说的是|指的是|就是|是|为)[\s“”‘’\"\']*',piece[occurrence.end():start]):return True
            return False
        for piece in pieces:
            ...
            if candidate in piece:
                ...
                if re.search(r'(?:不是|并非|非|not|never)\s*$',prefix,re.I):
                    continue
                if linked(piece,len(prefix),len(prefix)+len(candidate)):return True
            if label not in piece:
                continue
            if marker.search(piece):
                if re.search(r'(?:不是|并非|非|不是所说的|not|never)\s*(?:[^，,;；。.!?！？]*?)'+re.escape(label),piece,re.I):
                    continue
                if any(linked(piece,match.start(),match.end()) for match in marker.finditer(piece)):return True
        return False
    ...
    def apply(self,proposal,scope_id,now):
        ...
        if not mentioned:raise ContractError('DERIVATION_INVALID','reference_mention')
        episodes={self.tx.episodes.source_episode(s.ref,s.revision).ref for s in mentioned}
        if len(episodes)!=1:raise ContractError('DERIVATION_INVALID','reference_episode')
        ...
        resolved=set(); reason='model_candidates_only'
        for source in mentioned:
            if source_origin(source)!='human_direct' or source.capture_gaps:continue
            ...
            if ordinal_safe and re.search(r'(?:不是|并非|非|not|never)\s*'+re.escape(proposal['mention']),raw,re.I):
                ordinal_safe=False
            if snapshot.get('order')=='observed' and ordinal_safe:
                ...
                if index is not None:
                    ...
                    if version in proposal['candidate_refs']:resolved.add(version);reason='observed_display_order'
            for candidate,item in zip(proposal['candidate_refs'],candidates):
                ...
                unique_visible=(candidate in raw or self._visible_artifact_identity_count(item.label,item.revision)==1)
                exact=unique_visible and self._explicit_candidate_mention(raw,item.label,item.revision,candidate,proposal['mention'])
                if exact:resolved.add(candidate);reason='explicit_version_mention'
        choice=next(iter(resolved)) if len(resolved)==1 else None
        payload=dict(proposal,resolved_ref=choice,resolution='resolved' if choice else 'ambiguous' if len(candidates)>1 else 'unresolved')
        primary=mentioned[-1]
        ref='reference-'+hashlib.sha256(canonical([ctx.binding.installation_id,scope_id,ctx.project_id,ctx.branch_id,primary.ref,proposal['mention']]).encode()).hexdigest()
        ...
        if any(re.search(r'刚才|说的是|指的是|I meant|clarif',s.event['content'],re.I) for s in mentioned):
            matches=conn.execute('''SELECT r.reference_id FROM reference_bindings r JOIN reference_versions v
                ON v.reference_id=r.reference_id AND v.revision=r.current_revision WHERE r.episode_id=? AND r.read_blocked=0
                AND json_extract(v.payload_json,'$.mention')=? ORDER BY v.recorded_at DESC LIMIT 2''',(episode,proposal['mention'])).fetchall()
            if len(matches)==1:ref=matches[0][0]
        if not allowed(self.tx,'reference',ref):raise ContractError('SOURCE_MISSING')
        current=self.get(ref)
        stored=self._stored_head(ref)
        if current and current.payload==payload:return current
        revision=(stored[0]+1) if stored else 1
        if stored:
            conn.execute('UPDATE reference_bindings SET current_revision=? WHERE reference_id=?',(revision,ref))
            # No stale dependent summary remains eligible after a binding change.
            for row in conn.execute("SELECT DISTINCT object_ref FROM object_dependencies WHERE dependency_kind='reference' AND dependency_ref=? AND object_kind='episode'",(ref,)):
                conn.execute('UPDATE episode_versions SET processed_sequence=0 WHERE episode_id=?',(row[0],))
        else:
            conn.execute('INSERT INTO reference_bindings(reference_id,scope_id,project_id,branch_id,episode_id,current_revision,suppressed) VALUES (?,?,?,?,?,?,?)',
                (ref,scope_id,ctx.project_id,ctx.branch_id,episode,revision,int(any(s.suppressed for s in sources))))
        conn.execute('INSERT INTO reference_versions VALUES (?,?,?,?,?)',(ref,revision,canonical(payload),reason if choice else 'ambiguous_or_unconfirmed',now))
        for source in sources:
            conn.execute("INSERT INTO evidence_links VALUES ('reference',?,?,?,?,'derived_from','',NULL)",(ref,revision,source.ref,source.revision))
        for item in candidates:
            conn.execute("INSERT INTO object_dependencies VALUES ('reference',?,?,'artifact',?,?)",(ref,revision,item.ref,item.revision))
