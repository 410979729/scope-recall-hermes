...
_NEGATED = re.compile(r"(?:不要|别|不许|暂不)[^。！？!?;；\n]{0,4096}(?:删除|删掉|忘掉|清除)|\b(?:do not|don't)\s+(?:delete|erase|forget)\b",re.I)
...
def authorize_forget(tx,request,targets):
    ...
    raw = "\n".join(proofs)
    if _NEGATED.search(raw) or AUTHORITY_QUESTION.search(raw) or not (_DELETE.search(raw) if request["mode"]=="delete" else _SUPPRESS.search(raw)):
        raise ContractError("ACCESS_DENIED","forget_not_authorized")
