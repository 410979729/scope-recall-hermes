...
def repair_relation_rebuild_debt(
    ...
    *,
    ...
    """Inspect legacy debt and refuse the retired seed/drain mutation surface."""
    ...
    del scope_ids, max_events, pair_limit
    mutation_requested = bool(seed or drain)
    ...
    try:
        ...
        conn.close()
    legacy_debt = int(before.get("unresolved") or 0)
    frequency_debt = sum(
        int(frequency_before.get(key) or 0)
        for key in (
            "dirty_memories",
            "backfill_pending_scopes",
            "reclassification_pending_scopes",
        )
    )
    refused = [
        name
        for name, requested in (
            ("seed-relations", seed),
            ("drain-relations", drain),
        )
        if requested
    ]
    return {
        "ok": not mutation_requested,
        "status": (
            "legacy_mutation_disabled"
            if mutation_requested
            else "ready" if legacy_debt == 0 and frequency_debt == 0 else "debt"
        ),
        "reason_code": (
            "legacy_unbounded_relation_work_disabled"
            if mutation_requested
            else ""
        ),
        "dry_run": True,
        ...
        "operator_action": "use scripts/repair.relation_queue.py dry-run/apply",
    }
